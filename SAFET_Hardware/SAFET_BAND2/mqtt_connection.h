...
#define ALL_DATA_TOPIC "SAFET/band2/all_data"
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
    ...
    if (strcmp(topic, "SAFET/band2/ack") == 0) {
...
bool reconnect() {
    if (client.connect("Band2")) {
        client.subscribe(STATUS_TOPIC);
        client.subscribe("SAFET/band2/ack");  
    } else {
...
void mqttSketch(void * parameter) {
    ...
    for (;;) {
        ...
        client.publish("SAFET/band2/is_active", activeBuffer, activeSize);
        }
